...
class MeshKeypointCalibrator:
    ...
    def detect_mesh_orientation(self, mesh):
        ...
        Uses multiple heuristics for clothing meshes from TripoSR.
        ...
        print(f"  Mesh dimensions: X={size[0]:.3f}, Y={size[1]:.3f}, Z={size[2]:.3f}")
        
        # Heuristic 1: Height should be largest dimension for clothing
        # Find which axis has the largest extent
        axis_sizes = {'X': (size[0], 0), 'Y': (size[1], 1), 'Z': (size[2], 2)}
        tallest_axis_name, (tallest_size, tallest_idx) = max(axis_sizes.items(), key=lambda x: x[1][0])
        
        print(f"  Tallest dimension: {tallest_axis_name} ({tallest_size:.3f})")
        
        # Heuristic 2: Use PCA for refinement
        ...
        # Main axis should be aligned with maximum variance
        main_axis = eigenvectors[:, 0]
        
        print(f"  PCA main axis: [{main_axis[0]:.3f}, {main_axis[1]:.3f}, {main_axis[2]:.3f}]")
        
        # Target: Y-axis should be vertical (0, 1, 0)
        target_up = np.array([0, 1, 0])
        
        # Determine rotation needed
        transform = np.eye(4)
        
        # Check if tallest dimension is already Y
        if tallest_idx == 1:  # Y is already tallest
            # Check if it's pointing up or down
            y_alignment = np.dot(main_axis, target_up)
            
            if y_alignment > 0.7:  # Good alignment
            ...
            elif y_alignment < -0.7:  # Upside down
                print("  ⚠ Mesh is upside down, flipping...")
            ...
            # Need to rotate to make tallest axis become Y
            print(f"  ⚠ Need to rotate {tallest_axis_name} → Y")
            
            if tallest_idx == 0:  # X is tallest, need to rotate to Y
                # Rotate 90° around Z axis
                angle = -np.pi / 2
                axis = [0, 0, 1]
            elif tallest_idx == 2:  # Z is tallest, need to rotate to Y
            ...
            print(f"  Rotation: {np.degrees(angle):.1f}° around {axis}")
        
        orientation_info = {
            'needs_rotation': not np.allclose(transform, np.eye(4)),
            'tallest_axis': tallest_axis_name,
            'rotation_applied': transform,
            'original_size': size
        }
    ...
    def render_mesh_view(self, mesh, view='front', resolution=512):
        ...
        # Use matplotlib for reliable rendering as fallback
        try:
            # Try trimesh's scene rendering first
            scene = mesh.scene()
            ...
            print(f"⚠ Primary rendering failed: {e}")
            print("  Using improved projection method...")
            
            # IMPROVED FALLBACK: Create dense shaded rendering
            vertices = mesh.vertices
            faces = mesh.faces
            
            # Set up projection based on view
            if view == 'front':
                # Project to XY plane
                proj_indices = [0, 1]  # X, Y
                depth_index = 2  # Z for depth sorting
            elif view == 'side':
                # Project to YZ plane
                proj_indices = [2, 1]  # Z, Y
                depth_index = 0  # X for depth
            else:  # top
                # Project to XZ plane
                proj_indices = [0, 2]  # X, Z
                depth_index = 1  # Y for depth
            
            # Get projected coordinates
            x = vertices[:, proj_indices[0]]
            y = vertices[:, proj_indices[1]]
            depth = vertices[:, depth_index]
            ...
            # Center and scale to fit in image with padding
            padding = 50  # pixels
            scale = (resolution - 2 * padding) / max_range
            
            # Center in image
            x_center = (x_max + x_min) / 2
            y_center = (y_max + y_min) / 2
            
            # Apply transformation preserving aspect ratio
            x_norm = ((x - x_center) * scale + resolution / 2).astype(int)
            y_norm = ((y - y_center) * scale + resolution / 2).astype(int)
            
            # Flip Y for image coordinates (Y increases downward in images)
            y_norm = resolution - 1 - y_norm
            ...
            # Sort faces by average depth (back to front for proper occlusion)
            face_depths = []
            for face in faces:
                avg_depth = depth[face].mean()
                face_depths.append(avg_depth)
            
            sorted_face_indices = np.argsort(face_depths)
            ...
            for face_idx in sorted_face_indices:
                ...
                triangle = np.array([
                    [x_norm[face[0]], y_norm[face[0]]],
                    [x_norm[face[1]], y_norm[face[1]]],
                    [x_norm[face[2]], y_norm[face[2]]]
                ], dtype=np.int32)
                
                # Check if triangle is on screen
                if not (np.all(triangle[:, 0] >= 0) and np.all(triangle[:, 0] < resolution) and
                        np.all(triangle[:, 1] >= 0) and np.all(triangle[:, 1] < resolution)):
                    continue
                
                # Compute shading based on average depth
                shade = depth_norm[face].mean()
                color_val = int(100 + shade * 100)  # Range: 100-200 for gray clothing
                color = (color_val, color_val, color_val)
                
                # Draw filled triangle
                cv2.fillPoly(img_array, [triangle], color)
            
            # Add slight smoothing to reduce artifacts
            img_array = cv2.GaussianBlur(img_array, (3, 3), 0)
            
            image = Image.fromarray(img_array)
            print(f"✓ Created aspect-ratio-preserving projection view")
...
def capture_with_countdown(cap, countdown_seconds=5, message="Get ready!"):
    ...
    # Capture final frame WITHOUT any overlay text
    ret, frame = cap.read()
    if ret:
        frame = cv2.flip(frame, 1)
        # NO "CAPTURING!" overlay - just capture clean frame
...
def main():
    ...
    mesh_files = sorted(list(mesh_dir.glob("*_full.obj")))
    ...
    # Get reference depth map (environment only)
    print("Computing background depth map (environment reference)...")
    ...
    # Countdown and capture (clean frame without overlays)
    ...
    # Get user depth map (with person in scene)
    ...
    print(f"✓ User depth: range [0.0, 1.0], mean={depth_map_user.mean():.3f}")
    
    # IMPROVED APPROACH: Use user depth directly, not relative
    # The user depth map tells us where the user is in absolute camera space
    # We'll use this directly for keypoint Z-coordinates
    print("✓ Using user depth map for calibrated keypoint depth")
    ...
    # Create 3D keypoints using USER depth at keypoint positions
    print("Creating 3D skeleton with calibrated depth...")
    keypoints_3d_user = {}
    
    for name, (x, y) in keypoints_2d_user.items():
        # Get depth at this keypoint from USER depth map
        if 0 <= x < w and 0 <= y < h:
            z_depth = float(depth_map_user[y, x])
        else:
            z_depth = 0.5  # Default middle depth if out of bounds
        
        # Normalize coordinates to [-1, 1] range
        x_norm = (x / w) * 2 - 1
        y_norm = (y / h) * 2 - 1
        z_norm = z_depth * 2 - 1  # Map [0,1] to [-1,1]
        
        keypoints_3d_user[name] = (x_norm, y_norm, z_norm)
    
    print(f"✓ Created 3D skeleton with calibrated depth from user position")
    
    # Visualize calibration results
    print("\nGenerating calibration visualizations...")
    
    # Create visualization comparing background and user depth
    # Note: We're NOT using relative depth anymore
    ...
    # Add labels
    cv2.putText(vis_combined, "Background (Environment)", (50, 50),
               cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    cv2.putText(vis_combined, "User (Calibration Depth)", (w + 50, 50),
               cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    for name, (x, y) in keypoints_2d_user.items():
        # Get depth for color coding (from USER depth map)
        if 0 <= x < w and 0 <= y < h:
            z = depth_map_user[y, x]
            # Map depth to color: 0=far (blue), 1=close (red)
            color = (int(z * 255), 0, int((1 - z) * 255))  # Blue to red gradient
        else:
            color = (128, 128, 128)
    ...
    for i, mesh_path in enumerate(selected, 1):
        ...
        if orientation_info['needs_rotation']:
            ...
            size_before = bounds_before[1] - bounds_before[0]
            print(f"  Dimensions: X={size_before[0]:.3f}, Y={size_before[1]:.3f}, Z={size_before[2]:.3f}")
            ...
            print(f"  Dimensions: X={size_after[0]:.3f}, Y={size_after[1]:.3f}, Z={size_after[2]:.3f}")
            
            # Verify Y is now tallest
            if size_after[1] >= size_after[0] and size_after[1] >= size_after[2]:
                print("  ✓ Mesh is now upright (Y is tallest)")
            else:
                print("  ⚠ Warning: Y is not tallest after correction")
        else:
            print("✓ Mesh was already upright")
        ...
        if keypoints_2d_mesh is None:
            print("⚠ Could not detect pose in mesh rendering")
            print("  This can happen with complex clothing shapes.")
            
            choice = input("\nOptions:\n  1. Use user keypoints directly (recommended)\n  2. Skip this mesh\n  Choose (1/2): ").strip()
            
            if choice == '2':
                print("✗ Skipping mesh")
                continue
            
            print("\n✓ Using user keypoints as mesh keypoint reference")
            print("  The mesh will be aligned to your body position.")
            
            # Map user 2D keypoints to mesh space
            # We'll place them based on mesh bounds
            bounds = mesh.bounds
            mesh_center = bounds.mean(axis=0)
            mesh_size = bounds.ptp(axis=0)
            
            h_user, w_user = user_frame.shape[:2]
            keypoints_2d_mesh = {}
            
            # Convert user keypoints to mesh rendering space (512x512)
            for name, (x_user, y_user) in keypoints_2d_user.items():
                # Normalize user coordinates
                x_norm = x_user / w_user
                y_norm = y_user / h_user
                
                # Map to 512x512 mesh rendering space
                x_mesh = int(x_norm * 512)
                y_mesh = int(y_norm * 512)
                
                keypoints_2d_mesh[name] = (x_mesh, y_mesh)
            
            print(f"✓ Mapped {len(keypoints_2d_mesh)} user keypoints to mesh space")
        ...
        # Add coordinate axes visualization to help verify orientation
        h_vis, w_vis = vis_mesh.shape[:2]
        
        # Draw axis labels in corner
        axis_origin = (30, h_vis - 30)
        axis_scale = 20
        
        # X axis (red, horizontal right)
        cv2.arrowedLine(vis_mesh, axis_origin, 
                       (axis_origin[0] + axis_scale, axis_origin[1]),
                       (0, 0, 255), 2, tipLength=0.3)
        cv2.putText(vis_mesh, "X", (axis_origin[0] + axis_scale + 5, axis_origin[1] + 5),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        
        # Y axis (green, vertical up)
        cv2.arrowedLine(vis_mesh, axis_origin,
                       (axis_origin[0], axis_origin[1] - axis_scale),
                       (0, 255, 0), 2, tipLength=0.3)
        cv2.putText(vis_mesh, "Y", (axis_origin[0] - 10, axis_origin[1] - axis_scale - 5),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        
        # Add mesh info text
        cv2.putText(vis_mesh, f"Mesh: {mesh_path.name}", (10, 30),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        
        # Add orientation status
        if orientation_info['needs_rotation']:
            status = f"Corrected: {orientation_info['tallest_axis']} -> Y"
            color = (0, 255, 255)
        else:
            status = "Orientation: Upright"
            color = (0, 255, 0)
        
        cv2.putText(vis_mesh, status, (10, 60),
        ...
        calibration_data = {
            ...
            # Depth maps
            'depth_map_background': depth_map_background,  # Environment reference
            'depth_map_user': depth_map_user,  # User position depth (used for keypoints)
