...
# Fix for MPS interpolate bug in PyTorch 2.1.0
# See: https://github.com/pytorch/pytorch/issues/96610
os.environ['PYTORCH_ENABLE_MPS_FALLBACK'] = '1'

# Add TripoSR to path since we're running from tests/ directory
...
# Cross-platform device detection - FIXED for Mac
# Note: MPS has compatibility issues with TripoSR's Vision Transformer interpolation
# So we use CPU on Mac for stability (still reasonably fast for mesh generation)
if torch.cuda.is_available():
    # CUDA is available (PC with NVIDIA GPU)
    device = "cuda"
    logging.info("CUDA GPU detected, using CUDA")
else:
    # Mac or no GPU - use CPU
    # TripoSR works fine on Mac CPU for single mesh generation (5-15 seconds)
    device = "cpu"
    logging.info("Using CPU (Mac or no CUDA GPU)")
