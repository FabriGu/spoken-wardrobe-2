...
try:
    ...
    from VIBE.lib.utils.demo_utils import convert_crop_cam_to_orig_img
    from VIBE.lib.core.config import VIBE_DATA_DIR
    from VIBE.lib.models.smpl import SMPL, SMPL_MODEL_DIR
    from VIBE.lib.utils.geometry import batch_rodrigues, perspective_projection
...
def main():
    ...
    try:
        ...
        # )
        mesh_renderer = SMPLMeshRenderer(
            model_path='models/smplx',  # ← Let the library add /smplx/
            device=device
        )
    except Exception as e:
