"""
Day 2 (CORRECTED): OAK-D + BodyPix + Stable Diffusion Integration

FIXES:
1. Captures CLEAN frame (before drawing skeleton/text)
2. Uses BodyPix for precise body part segmentation
3. Runs BodyPix on OAK-D RGB frame (not cv2.VideoCapture)
...
def main():
    print("="*70)
    print("Day 2 (CORRECTED): OAK-D + BodyPix + SD Integration")
    print("="*70)
    print("\nFIXES:")
    print("  ✓ Uses CLEAN frame (no skeleton/text overlay)")
    print("  ✓ Uses BodyPix for precise body part masks")
    print("  ✓ Runs BodyPix on OAK-D RGB frame")
    print("\nControls:")
    ...
    while True:
        ...
        cv2.imshow("OAK-D + BodyPix + SD", display_frame)
